...
Next.js의 App Router에서는 RSC를 사용하면 Streaming HTML을 사용할 수 있다.
...
**Streaming SSR**은 서버 측에서 렌더링을 점진적으로 진행하는 방식으로, 사용자에게 빠르게 페이지를 표시할 수 있도록 돕는 기술이다. 주요 특징은 **가장 빠르게 렌더링할 수 있는 부분**을 우선적으로 표시하고, 나머지 부분은 서버에서 순차적으로 스트리밍하는 것이다. 이 과정에서 **Suspense**, **코드 분할**, **스트리밍 HTML**, **선택적 하이드레이션**과 같은 기술이 활용된다.
...
React 18은 Suspense와 통합하여 **스트리밍 HTML**과 **선택적 하이드레이션** 기능을 사용할 수 있다. 이러한 기능은 React에 기존에 없던 동시성을 부여한다고 볼 수 있다. 이러한 기능과 RSC를 통합하면 렌더링 과정에서 병렬성을 활용해 렌더링 퍼포먼스를 비약적으로 향상할 수 있다.
